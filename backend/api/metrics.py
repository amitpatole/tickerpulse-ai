...
def get_summary():
    ...
    try:
        ...
        logger.error("Database query failed in get_summary: %s", exc)
        raise DatabaseError("Failed to retrieve metrics summary.") from exc
...
def get_agent_metrics():
    ...
    try:
        ...
        logger.error("Database query failed in get_agent_metrics: %s", exc)
        raise DatabaseError("Failed to retrieve agent metrics.") from exc
...
def get_system_metrics():
    ...
    try:
        ...
        raise DatabaseError("Failed to retrieve system metrics.") from exc
        ...
        'snapshots': [
            {
                ...
                'db_pool_in_use': row['db_pool_in_use'],
    ...
    try:
        with pooled_session() as conn:
            if metric == 'cost':
                ...
                # Optional ?agent= filter pushes the predicate into SQL so the
                # covering index idx_agent_runs_duration_cov eliminates a full
                # scan.  A hard LIMIT 50000 caps memory use regardless of range.
        ...
        logger.error("Database query failed in get_timeseries: %s", exc)
        raise DatabaseError("Failed to retrieve timeseries data.") from exc
...
def get_job_metrics():
    ...
    try:
        ...
        logger.error("Database query failed in get_job_metrics: %s", exc)
        raise DatabaseError("Failed to retrieve job metrics.") from exc
